...
function createMutationInput(modelName, model, gqlFields, prefix, allOptional = false) {
  ...
  Object.keys(gqlFields).forEach((fieldName) => {
    ...
    if (sqlField) {
      if (!sqlField._autoGenerated && !sqlField.autoIncrement) { //eslint-disable-line
        ...
        if (gqlField.type instanceof GraphQLNonNull && sqlField.defaultValue) {
          fields[fieldName] = {type: gqlField.type.ofType};
        } else if (gqlField.type instanceof GraphQLNonNull && !sqlField.defaultValue) {
          fields[fieldName] = {type: new GraphQLNonNull(gqlField.type.ofType)};
        } else {
          fields[fieldName] = {type: gqlField.type};
        }
      }
    }
  });
      ...
      if (model.relationships) {
        Object.keys(model.relationships).forEach(async(relName) => {
          ...
          if (targetType) {
            ...
            const input = new GraphQLInputObjectType(inputOptions);
            switch (relationship.type) {
